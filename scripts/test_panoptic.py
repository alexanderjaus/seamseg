...
parser.add_argument(
    "--score_threshold", type=float, default=0.5, help="Detection confidence threshold"
)
parser.add_argument(
    "--iou_threshold", type=float, default=0.5, help="Panoptic disambiguation IoU threshold"
)
parser.add_argument(
    "--min_area", type=float, default=4096, help="Minimum pixel area for stuff predictions"
)
parser.add_argument(
    "--raw", action="store_true", help="Save raw predictions instead of rendered images"
)
...
def make_dataloader(args, config, rank, world_size):
    ...
    test_tf = ISSTestTransform(
        config.getint("shortest_size"), config.getstruct("rgb_mean"), config.getstruct("rgb_std")
    )
    test_db = ISSTestDataset(args.data, test_tf)
    test_sampler = DistributedARBatchSampler(
        test_db, config.getint("val_batch_size"), world_size, rank, False
    )
    test_dl = data.DataLoader(
        test_db, batch_sampler=test_sampler, collate_fn=iss_collate_fn, pin_memory=True
    )
...
def make_model(config, num_thing, num_stuff, extra_resolutions):
    ...
    return PanopticNet(
        body, rpn_head, roi_head, sem_head, rpn_algo, roi_algo, sem_algo, classes, extra_resolutions
    )
...
def test(model, dataloader, **varargs):
    ...
    for it, batch in enumerate(dataloader):
        with torch.no_grad():
            ...
            for i, (sem_pred, bbx_pred, cls_pred, obj_pred, msk_pred) in enumerate(
                zip(
                    pred["sem_pred"],
                    pred["bbx_pred"],
                    pred["cls_pred"],
            ...
            if varargs["summary"] is not None and (it + 1) % varargs["log_interval"] == 0:
                logging.iteration(
                    ...
                    it + 1,
                    len(dataloader),
                    OrderedDict([("data_time", data_time_meter), ("batch_time", batch_time_meter)]),
                )
...
def main(args):
    ...
    # Create model
    model = make_model(config, meta["num_thing"], meta["num_stuff"], [2000, 2500, 3000, 3500, 4000])
    ...
    model = DistributedDataParallel(
        model.cuda(device), device_ids=[device_id], output_device=device_id
    )

    # Panoptic processing parameters
    panoptic_preprocessing = PanopticPreprocessing(
        args.score_threshold, args.iou_threshold, args.min_area
    )
